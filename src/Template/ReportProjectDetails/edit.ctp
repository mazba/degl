...
<div class="breadcrumb-line">
    <ul class="breadcrumb">
        ...
        <li><?= $this->Html->link(__('Offices'), ['action' => 'index']) ?></li>
                    <li class="active"><?= __('Edit Office') ?></li>
...
<div class="tabbable page-tabs">
    <ul class="nav nav-tabs">
        <li><?= $this->Html->link(__('List of Offices'), ['action' => 'index']) ?></li>
            ...
            ?>
            <li class="active"><?= $this->Html->link(__('Edit Office'), ['action' => 'edit', $office->id
                ]) ?>
            </li>
            <?php
            if ($user_roles['delete'] == 1)
            {
                ...
                <li><?=
                    ...
                    ['confirm' => __('Are you sure you want to delete # {0}?', $office
                    ->id)]
                    )
                    ?>
                </li>
            ...
            {
                ?>
                <li><?= $this->Html->link(__('Details Office'), ['action' => 'view', $office->id])
                    ?>
                </li>
            <?php
            }
...
<div class="panel panel-default">
    ...
    <div class="panel-body col-sm-6">
        ...
        ?>
        <div id="main_container_zone_dropdown" style="<?php echo (sizeof($zones)>0)?'':'display: none';?>">
            <?php
            echo $this->Form->input('zone_id', ['options' => $zones, 'empty' =>  __('SELECT_ZONE')]);
            ?>
        </div>
        <div id="main_container_district_dropdown" style="<?php echo (sizeof($districts)>0)?'':'display: none';?>">
            <?php
            echo $this->Form->input('district_id', ['options' => $districts, 'empty' =>  __('SELECT_DISTRICT')]);
            ?>
        </div>
        <div id="main_container_upazila_dropdown" style="<?php echo (sizeof($upazilas)>0)?'':'display: none';?>">
            <?php
            echo $this->Form->input('upazila_id', ['options' => $upazilas, 'empty' =>  __('SELECT_UPAZILA')]);
            ?>
        </div>
    </div>
    <div class="panel-body col-sm-6">

...
<div class="breadcrumb-line">
    <ul class="breadcrumb">
        ...
        <li class="active"><?= __('Receive File Registers') ?></li>
    </ul>
</div>
<div class="tabbable page-tabs">
    <ul class="nav nav-tabs">
        <li class="active"><?= $this->Html->link(__('List of Receive File Registers'), ['action' => 'index']) ?></li>
        <?php
        if ($user_roles['add'] == 1)
        {
            ?>
...
//                selectionmode: 'checkbox',
                ...
                columns: [
                    ...
                    { text: '<?= __('Office Name') ?>', dataField: 'sender_office_name',width:'20%'},
                    { text: '<?= __('Address') ?>',dataField: 'sender_address',width:'20%'},
                    { text: '<?= __('Subject') ?>',dataField: 'subject',width:'17%'},
                    { text: '<?= __('Date') ?>', dataField: 'receive_date',width:'10%'},
        ...
        $(document).on("click", ".print", function(event)
        {
            ...
            $.ajax({
                url: '<?=$this->Url->build(('/ReceiveFileRegisters/print_it'), true)?>',
                type: 'POST',
                data:{
                    type:req_type,

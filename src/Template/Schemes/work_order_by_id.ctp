...
if(!empty($empty_array)){
	...
	foreach($empty_array as $value) {
		if ($value == 'work_order_date') {$str .= 'ওয়ার্ক অর্ডার তারিখ, ';}
		if ($value == 'work_name') {$str .= 'কাজের নাম, ';}
		if ($value == 'etender_no') {$str .= 'e-Tender ID No, ';}
		if ($value == 'estimation_sarok_no') {$str .= 'প্রাক্কলন অনুমোদনের স্মারক নং, ';}
		if ($value == 'estimation_date') {$str .= 'প্রাক্কলন অনুমোদনের তারিখ, ';}
		if ($value == 'etender_notice') {$str .= 'e-Tender দরপত্র বিজ্ঞপ্তি নং, ';}
		if ($value == 'e_tender_date') {$str .= 'e-Tender দরপত্র বিজ্ঞপ্তি  তারিখঃ, ';}
		if ($value == 'obtain_tender_no') {$str .= 'প্রাপ্ত দরপত্র সংখ্যা, ';}
		if ($value == 'customary_tender_no') {$str .= 'রীতিসিদ্ধ দরপত্র সংখ্যা, ';}
		if ($value == 'performance_security') {$str .= 'Performance Security, ';}
		if ($value == 'tender_date') {$str .= 'দরপত্র গ্রহণের তারিখ, ';}
		if ($value == 'applied_tender_price') {$str .= 'দাখিলকৃত দরপত্র মূল্য, ';}
		if ($value == 'project_name') {$str .= 'প্রকল্পের নাম, ';}
		if ($value == 'nothi_name') {$str .= 'নথি নং, ';}
		if ($value == 'name_en') {$str .= 'Contract Package No, ';}
		if ($value == 'contractor_title') {$str .= 'নির্বাচিত ঠিকাদার, ';}
		if ($value == 'order_number') {$str .= 'পে অর্ডার নং, ';}
		if ($value == 'initial_date') {$str .= 'পে অর্ডার তারিখ, ';}
		if ($value == 'expire_date') {$str .= 'মেয়াদ তারিখ, ';}
		if ($value == 'submit_date') {$str .= 'দাখিল তারিখ, ';}
		if ($value == 'order_medium') {$str .= 'e-GP Medium, ';}
		if ($value == 'order_branch') {$str .= 'দাখিল শাখা, ';}
	}
}
?>
...
<div id="PrintArea">
	<!--	--><?php //if(in_array_any(['', '0'],$result)): ?>
	<?php if(!empty($empty_array)): ?>
		<h3 class="text-center"><?= $str ?><?= __('এই তথ্য প্রদান করা হয় নাই। সেহেতু আপনি ওয়ার্ক অর্ডার দেখতে পারবেন না') ?></h3>
	<?php else: ?>

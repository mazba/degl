...
<div class="row panel panel-default">
    ...
    <div class="panel-body col-sm-12">
        <div class="well" style="border:1px solid #003399;margin-bottom: 20px">
            ...
            <?php
            if (true) { ?>
                 <a class="btn btn-primary" href="<?= $this->Url->build(('/proposed_ra_bills'), true); ?>/view/<?= $messages->resource_id; ?>">View RA Bill</a>
                 <a class="btn btn-success" href="<?= $this->Url->build(('/ra_bill_application'), true); ?>/view_measurement/<?= $messages->resource_id; ?>">View Measurement</a>

                <?php if($log_user['office_id']==1 && $log_user['user_group_id']== 2){?>
                 <a class="btn btn-danger" href="<?= $this->Url->build(('/ra_bill_application'), true); ?>/approve_ra_bill/<?= $messages->resource_id; ?>">Approve RA Bill</a>
          <?php }?><br/>
            <?php } ?>
            ...
            <?php if ($attachments) { ?>
                <div class="form-group">
                    <label><?= __('Attachment') ?></label></br>
                    RA Bill:
                    <?php foreach ($attachments as $data) { ?>

                    <?php if ($data->file_label=="rabill"){?>

                        <a href="<?php echo Router::url('/', true) . 'files/receive_files/' . $data->file_path; ?>"><?php echo $data->file_path; ?></a></br>
                    ...
                    <?php
                    if ($data->file_label=="application"){?>
    ...
    <div id="forward" class="col-md-12" style="display: none">
        <?= $this->Form->create(null, ['url' => ['controller' => 'RaBillApplication', 'action' => 'view/' . $messages->id], 'class' => 'form-horizontal', 'role' => 'form']); ?>
    ...
    <div id="reply" class="col-md-12" style="display: none">
        <?= $this->Form->create(null, ['url' => ['controller' => 'RaBillApplication', 'action' => 'view/' . $messages->id], 'class' => 'form-horizontal', 'role' => 'form', 'type' => 'file']); ?>

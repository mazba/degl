 ...
 */
class MultimediaImageController extends AppController
{
    ...
    public function index()
    {
        ...
        $schemes = $multimedia->find('all')
            ...
            ->where(['multimedia.office_id'=>$user['office_id'],'multimedia.type'=>'image'])
            ->toArray();
        ...
        if ($this->request->is(['patch', 'post', 'put']))
        {
            ...
            for($i=0; $i<sizeof($_FILES['files']['name']); $i++)
            {
                if(!substr($_FILES["files"]["type"][$i],0,5) == 'image')
                {
                    ...
                    $this->Flash->error(__('File types is not valid. Please Upload a Image file'));
                    break;
                }
            }
            if($file_type_valid)
            {
                $base_upload_path = WWW_ROOT.'files/multimedia_images';
                for($i=0; $i<sizeof($_FILES['files']['name']); $i++)
                {
                ...
                if($file_upload_complete)
                {
                    ...
                    $data['date'] = strtotime($inputs['image_capture_date']);
                    $data['type'] = 'image';
                    ...
                    $this->Flash->success(__('The Multimedia Image has been saved.'));
                    return $this->redirect(['action' => 'index']);
                }
                else
                {
                    $this->Flash->error(__('The Multimedia Image could not be saved. Please, try again.'));
                }
            }
        }

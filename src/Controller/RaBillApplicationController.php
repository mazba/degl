...
class RaBillApplicationController extends AppController
{
...
//echo "<pre>";print_r($messages);die();
        ...
        if ($this->request->is('post')) {
          ...
          //  echo "<pre>";print_r($_FILES);die();
            ...
            $data['msg_type'] = 'RaBillApplication';
        ...
        $this->set(compact('departments', 'messages', 'attachments', 'reply','log_user'));
    }
    ...
    public function add()
    {
        ...
        // echo "<pre>";print_r($inputs);die();
            ...
            $temp = $this->proposed_ra_bills->get($inputs['resource_id']);


            $data = array();
            $data['sender_id'] = $user['id'];
            $data['subject'] = $inputs['subject'];
            $data['scheme_id'] = $temp->scheme_id;
            ...
            foreach ($inputs['user'] as $user_id) {
                ...
                if ($has_file) {
                    ...
                    foreach ($files as $key=>$file) {
                        $file_data = array();
                        $file_data['file_path'] = $file['file_path'];
                        $file_data['file_label'] = $inputs['file_label'][$key];
                        $file_data['table_name'] = 'message_registers';
                        $file_data['table_key'] = $msg['id'];
                        $file_data['created_by'] = $user['id'];
                        $file_data['created_date'] = time();
        ...
        $this->loadModel('Departments');
     //   $departments = $this->Departments->find('all', ['contain' => ['Users', 'Users.Designations'], 'conditions' => ['Departments.office_id' => $user['office_id']]]);
        $departments = $this->Departments->find('all', ['contain' => ['Users', 'Users.Designations']]);

        $this->loadModel('ProposedRaBills');
     //   $departments = $this->Departments->find('all', ['contain' => ['Users', 'Users.Designations'], 'conditions' => ['Departments.office_id' => $user['office_id']]]);
        $RaBills = $this->ProposedRaBills->find('all', ['contain' => ['Schemes']]);
           // echo "<pre>";print_r($RaBills->toArray());die();
        ...
        $this->set(compact('departments', 'projects', 'schemes','RaBills'));
    }
...
//                ->orWhere(['MessageRegisters.msg_type' => 'hireCharges'])
            ...
            foreach ($messages as $message) {
                if ($message->thread_id) {
                    $message['action'] = '<a class="icon-newspaper" href="' . $this->request->webroot . 'RaBillApplication/view/' . $message['thread_id'] . '" ><a>';
                } else {
                    $message['action'] = '<a class="icon-newspaper" href="' . $this->request->webroot . 'RaBillApplication/view/' . $message['id'] . '" ><a>';
                }
        ...
        if ($action == 'sent') {
            ...
            $this->loadModel('MessageRegisters');
            $messages = $this->MessageRegisters->find()
                ->where(['MessageRegisters.sender_id' => $user['id'], 'MessageRegisters.msg_type' => 'RaBillApplication', 'MessageRegisters.attachment_type' => Configure::read('attachment_type.0')])
    ...
    public function view_measurement($id){
      //  echo $id;
        ...
        $measurementBook = $this->proposed_ra_bills->get($id);
       //echo "<pre>";print_r($measurementBook);die();

        $measurement_info=TableRegistry::get('Measurements')->find()
            ...
            -> where(['Measurements.scheme_id'=>$measurementBook['scheme_id'],'Measurements.measurement_no'=>$measurementBook['measurement_no']])
        ->leftJoin('schemes_items', 'schemes_items.id=Measurements.item_id');
        $this->set('measurement_info', $measurement_info);

    }

    public function approve_ra_bill($id){
        $user = $this->Auth->user();
        ...
        if ($this->request->is('post')) {
            ...
            $inputs = $this->request->data;
         //   echo "<pre>";print_r($inputs);die();
            $query = $this->proposed_ra_bills->query()
            ->update()
               // ->set(['status' => 0])
                ->set(['approve_bill_amount' => $inputs['approve_bill_amount']])
                ->where(['id' => $id])
                ->execute();
            ...
            if ($query) {
                $this->Flash->success(__('The  ra bill has been saved.'));
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('The  ra bill could not be saved. Please, try again.'));
            }


        }

        $measurementInfo = $this->proposed_ra_bills->get($id);
        $this->loadModel('Departments');
        $departments = $this->Departments->find('all', ['contain' => ['Users', 'Users.Designations'], 'conditions' => ['Departments.office_id' => $user['office_id']]]);

        $this->set('measurementInfo', $measurementInfo);
        $this->set('id',$id);

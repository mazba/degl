...
class VehicleServicingsController extends AppController {
  ...
  public function view($id = NULL) {
    $vehicleServicing = $this->VehicleServicings->get($id, ['contain' => ['Offices', 'Vehicles', 'VehicleServicingDetails']]);
    //echo "<pre>";print_r($vehicleServicing);

    $this->loadModel('VehiclesStatus');
    $VehicleStatus = $this->VehiclesStatus->find('all')
                                          ->where(['vehicle_id' => $vehicleServicing->vehicle_id, 'VehiclesStatus.status' => 1])
                                          ->contain(['Employees'])
                                          ->first();
    //echo "<pre>";print_r($VehicleStatus);die();
    $this->set('vehicleServicing', $vehicleServicing);
    $this->set('VehicleStatus', $VehicleStatus);
    $this->set('_serialize', ['vehicleServicing', 'VehicleStatus']);
  }
...
//             die;
  ...
  public function edit($id = NULL) {
    $user = $this->Auth->user();
    //$vehicleServicing = $this->VehicleServicings->get($id, ['contain' => ['Vehicles']]);
    $vehicleServicing = $this->VehicleServicings->get($id, ['contain' => ['Offices', 'Vehicles', 'VehicleServicingDetails']]);
    ...
    if ($this->request->is(['patch', 'post', 'put'])) {
      ...
      if ($this->VehicleServicings->save($vehicleServicing)) {
        ...
        $this->loadModel('VehicleServicingDetails');
        $this->VehicleServicingDetails->deleteAll(['vehicle_servicing_id' => $id]);
        foreach ($parts as $part) {
          $vehicleSericingDetail = $this->VehicleServicingDetails->newEntity();
          $vehicleSericingDetail->vehicle_servicing_id = $vehicleServicing->id;
          $vehicleSericingDetail->name = $part['name'];
  ...
  public function ajax($action = NULL) {
    if ($action == 'get_grid_data' && $this->user_roles['index']) {
      ...
      if ($user['user_group_id'] == 1) {
        $vehicleServicings = $this->VehicleServicings->find('all', ['conditions' => ['VehicleServicings.status' => 1], 'contain' => ['Offices', 'Vehicles'], 'order' => ['VehicleServicings.id' => 'ASC']

        ])
                                                     ->toArray();
      } else {
        $vehicleServicings = $this->VehicleServicings->find('all', ['conditions' => ['VehicleServicings.status' => 1, 'VehicleServicings.office_id' => $user['office_id']], 'contain' => ['Offices', 'Vehicles'], 'order' => ['VehicleServicings.id' => 'ASC']])
                                                     ->toArray();
      }

        ...
        if ($action == 'get_grid_data' && $this->user_roles['index']) {
            ...
            if ($user['user_group_id'] == 1) {
                ...
                foreach ($schemes as &$scheme) {
                    ...
                    $scheme['action'] = '<button title="' . __('Edit') . ' " data-scheme_id="' . $scheme['scheme_id'] . '" class="icon-newspaper text-danger edit" > </button>' . '' . '&nbsp;<a class="" title="Assign Contractors" href="' . $this->request->webroot . 'Schemes/assign_contractors/' . $scheme['scheme_id'] . '" ><i class="icon-user-plus"></i><a>';
...
//        ]);
        ...
        $scheme = $this->Schemes->get($id, [
            'contain' => ['Projects', 'WorkTypes', 'WorkSubTypes', 'Districts', 'Upazilas', 'Municipalities', 'FinancialYearEstimates', 'SchemeDetails']
        ]);
...
//            echo "<pre>";print_r($scheme);die();
        ...
        $this->loadModel('SchemeContractors');
        $assigned_contractors = $this->SchemeContractors->find('all', ['contain' => ['Contractors'], 'conditions' => ['scheme_id' => $id]])->toArray();
        //echo "<pre>";print_r($assigned_contractors);die();

        $this->loadModel('Files');
        $contractor_file = $this->Files->find('all', ['conditions' => ['table_key' => $id]])->toArray();
    ...
    public function edit_scheme_progress($scheme_id)
    {
        ...
        $id = $scheme_id;
        $query = TableRegistry::get('SchemeProgresses')->find()
            ->select(['progress_value'])
            ->where(['scheme_id' => $scheme_id, 'status' => 1]);
        $previous_scheme_progresses = $query->first();

        $query = TableRegistry::get('SchemeProgressPlans')->find()
            ->where(['scheme_id' => $scheme_id]);
        // ->order(['date' => 'DESC']);
        $previous_scheme_progress_plans = $query->toArray();

        $query = TableRegistry::get('scheme_progresses')->find()
            ->where(['scheme_id' => $scheme_id]);
    ...
    public function edit_item_bbq($scheme_id)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            foreach($inputs['item'] as $itm){
                $item_info = TableRegistry::get('scheme_items_info')->find()
        ...
        $estimated_itms = $this->estimated_schemes_items->find()->where(['scheme_id' => $scheme_id]);
        $this->set('items', $itms);
...
//        $this->set('items', $itms);
    ...
    public function scheme_vehicle_manage($scheme_id)
    {
        ...
        $hire_charges = TableRegistry::get('hire_charges')->find()
            ->where(['scheme_id' => $scheme_id]);
        $details = [];
        foreach ($hire_charges as $row) {
            $details[$row['id']]['info'] = $hire_charge_details = TableRegistry::get('hire_charge_details')->find()
...
//                }
        ...
        $measurement_head = TableRegistry::get('Measurements')->find()
        ...
        foreach ($measurement_head as $measurement_no) {
            $measurement_details = TableRegistry::get('Measurements')->find()
        ...
        $items = TableRegistry::get('SchemesItems')->find()
    ...
    public function get_lab_grid_data($scheme_id){
        ...
        $labLetterRegisters = $this->LabLetterRegisters->find('all', [
            'conditions' =>['scheme_id'=>$scheme_id,'LabLetterRegisters.status !=' => 99],'order'=>['LabLetterRegisters.id'=>'DESC']])
            ->toArray();

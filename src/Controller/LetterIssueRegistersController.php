...
class LetterIssueRegistersController extends AppController
{
    ...
    public function view($id = null)
    {
        $letterIssueRegister = $this->LetterIssueRegisters->get($id, [
            'contain' => ['Projects', 'Schemes', 'CreatedUser', 'UpdatedUser', 'NothiRegisters']
        ]);
...
//                $data['letter_no']=$inputs['letter_no'];
                ...
                if ($status = $this->LetterIssueRegisters->save($letterIssueRegister)) {
                    ...
                    foreach ($files as $file) {
                        ...
                        $file_data['table_name'] = 'letter_issue_registers';
    ...
    public function ajax($action = null)
    {
        if ($action == 'get_grid_data') {
            ...
            if ($user['user_group_id'] == 1) {
                $letterIssueRegisters = $this->LetterIssueRegisters->find('all', [
                    'conditions' => ['LetterIssueRegisters.status !=' => 99],
                ])->toArray();
            } else {
                $letterIssueRegisters = $this->LetterIssueRegisters->find('all', [
                    'conditions' => ['LetterIssueRegisters.status !=' => 99, 'sender_office' => $user['office_id']],
                ])->toArray();
            }
    ...
    public function print_it()
    {
        ...
        if ($this->request->data('type') == 'by_date') {
            ...
            if ($user['user_group_id'] == 1) {
                $letterIssueRegisters = $this->LetterIssueRegisters->find('all', [
                    'conditions' => [
                        'LetterIssueRegisters.status !=' => 99,
                        'LetterIssueRegisters.issue_date >=' => $start_date,
                        'LetterIssueRegisters.issue_date <=' => $end_date,
                    ],
                ])->toArray();
            } else {
                $letterIssueRegisters = $this->LetterIssueRegisters->find('all', [
            ...
            if ($user['user_group_id'] == 1) {
                $letterIssueRegisters = $this->LetterIssueRegisters->find('all', [
                    'conditions' => [
                        'LetterIssueRegisters.status !=' => 99,
                        'LetterIssueRegisters.issue_date >=' => strtotime('00:00'),// TODO:check time issue

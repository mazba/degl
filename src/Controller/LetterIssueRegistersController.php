  ...
  public function ajax($action = NULL) {
    if ($action == 'get_grid_data') {
      ...
      if ($user['user_group_id'] == 1) {
        $letterIssueRegisters = $this->LetterIssueRegisters->find('all', ['conditions' => ['LetterIssueRegisters.status !=' => 99],])
                                                           ->toArray();
      } else {
        $letterIssueRegisters = $this->LetterIssueRegisters->find('all', ['conditions' => ['LetterIssueRegisters.status !=' => 99, 'sender_office' => $user['office_id']],])
                                                           ->toArray();
      }
